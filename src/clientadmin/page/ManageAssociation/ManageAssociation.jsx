                                            ...
                                            <div className="col-12 col-xl-4">
                                                <div className="input-group">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder="Search by Association ID..."
                                                        value={searchQuery}
                                                    ...
                                                    {filterAssociations(associations).length > 0 ? (
                                                        filterAssociations(associations).map((association, index) => (
                                                            <tr key={association._id || index}>
                                                                ...
                                                                <td>{association.created_by}</td>
                                                                <td>{association.created_date ? new Date(association.created_date).toLocaleString() : ''}</td>
                                                                <td style={{ color: association.status ? 'green' : 'red' }}>{association.status ? 'Active' : 'Inactive'}</td>
                                                                <td>
                                                                    <button
                                                                        type="button"
                                                                    ...
                                                                    <button
                                                                        type="button"
                                                                        className={`btn btn-outline-${association.status ? 'danger' : 'success'} btn-icon-text`}
                                                                        onClick={() => deactivateAssociation(association.association_id, association.status)}
                                                                        style={{ marginBottom: '10px', marginRight: '10px' }}
                                                                    >
                                                                        <i className={`mdi mdi-${association.status ? 'delete' : 'check'} btn-icon-prepend`}></i>{association.status ? 'DEACTIVATE' : 'ACTIVATE'}
                                                                    </button>
                                                                </td>
                                                                <td>
                                                                <button
                                                                        type="button"
                                                                        className="btn btn-outline-dark btn-icon-text"
                                                                        onClick={() => navigatetochargerdetails(association)}
                                                                        style={{ marginBottom: '10px', marginRight: '10px' }}
                                                                    >
                                                                        <i className="mdi mdi-eye btn-icon-prepend"></i>View
                                                                    </button>
